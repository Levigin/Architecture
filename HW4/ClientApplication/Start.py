...
# The main class off client application
class Start(EnterData):
    ...
    def __login(self) -> None:
        ...
        customer = Customer()
        try:
            self.customer.set_user(
                Authentication.authentication(self.customer.get_user_provider(), user_name, password_hash))
        except RuntimeError as e:
            print("FAIL")
            print(e)
            print("=====================================================================================")
    ...
    def __register(self) -> None:
        ...
        card_number = self._input_int(1, 9999_9999_9999_9999)
        print("=====================================================================================")
        ...
        self.__print_message_line(
            "OK. user " + self.customer.get_user().get_user_name() + " with ID " + str(id) + "added to base.")
    ...
    def __run_buying_menu(self) -> None:
        ...
        while run:
            ...
            self.__print_message_line("To select route number and print all tickets\tenter 1\n" +
                                      "To logout\t\t\t\t\t\t\t\t\t\tenter 0")
            print("Enter your choice > ")
            try:
    ...
    def __run_buying_menu_choice_logic(self, choice: int) -> bool:
        match choice:
            case 1:
                ticket_route_number = self.__run_select_route_menu()
                if ticket_route_number > 0:
                    ticket_date = self.__run_select_date()
                    if ticket_date is not None:
                        try:
                            self.customer.setSelectedTickets(
                                self.customer.search_ticket(ticket_date, ticket_route_number))
                        except RuntimeError as e:
                            self.__print_message_line(str(e))
                            return True
                        self.__print_all_tickets(self.customer.get_selected_tickets())
    ...
    def __buy_ticket_menu(self):
        self.__print_message_line("Confirm to buy. | User " + self.customer.getUser().getUserName() + " |")
        print(
    ...
    def __buy_ticket_menu_confirm_logic(self, answer: str):
        if answer.lower() == "YES".lower():
            for t in self.customer.getSelectedTickets():
                if t.getDate().equals(
                        self.ticket_date) and t.get_route_number() == self.ticket_route_number and t.get_valid():
                    flag = False
                try:
